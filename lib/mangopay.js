...
const debug = require('debug')('mangopay-sdk');
...
class Mangopay {
    ...
    constructor (username, password, opts) {
        ...
        this.PayIn = {
            ...
            DirectDebit: {
                ...
                Direct: {
                    create: (userId, mandateId, data) => this._createDirectDebitViaDirectPayIn(userId, mandateId, data)
                }
            },
            PreAuthorized: {
                Direct: {
        ...
        this.Mandate = {
            get: id => this._getMandate(id),
            list: () => this._listMandates()
        };
    }
    ...
    _request (method, path, opts) {
        ...
        return _request({ method, uri: `${ this.config.mangopayApiUrl }/${ this.config.apiVersion }/${ this._username }/${ path }`,
            ...
            .spread((response, body) => {
                if(_opts.body) {
                    debug("Request body: "+JSON.stringify(_opts.body));
                }
                debug("Mangopay reply: "+JSON.stringify(body));
    ...
    _bindUser (user) {
        ...
        user.BankAccount = {
            get: (bankAccountId) => this._getUserBankAccount(user.Id, bankAccountId),
        ...
        user.Mandate = {
            list: () => this._listUserMandates(user.Id)
        };

        user.PayIn = {
            DirectDebit: {
                Direct: {
                    create: (mandateId, data) => this._createDirectDebitViaDirectPayIn(user.Id, mandateId, data)
                }
            }
        };
    ...
    _bindBankAccount (bankAccount, userId) {
        ...
        bankAccount.update = (data) => this._updateBankAccount(userId, bankAccount.Id, data);
        bankAccount.Mandate = {
            create: data => this._createMandate(bankAccount.Id, data),
            list: () => this._listBankAccountMandates(userId, bankAccount.Id)
        };
        return bankAccount;
    }

    _bindMandate (mandate) {
        mandate.reload = () => this._getMandate(mandate.Id);
        mandate.cancel = () => this._cancelMandate(mandate.Id);
    ...
    _updateBankAccount (userId, bankAccountId, _data) {
        return validator(_data, this._validationSchemas.bankAccountOther)
            .then(data => this._request('PUT', `users/${ userId }/bankAccounts/${ bankAccountId }`, {body: data}))
            .then(bankAccount => this._bindBankAccount(bankAccount, userId));
    }
    ...
    _getMandate (mandateId) {
        return this._request('GET', `mandates/${ mandateId }`)
            .then(mandate => this._bindMandate(mandate));
    }

    _listMandates() {
        return this._request('GET', 'mandates')
            .map(mandate => this._bindMandate(mandate));
    }

    _listUserMandates(userId) {
        return this._request('GET', `users/${ userId }/mandates`)
            .map(mandate => this._bindMandate(mandate));
    }

    _createMandate(bankAccountId, data) {
        var clonedData = _.cloneDeep(data); //This may be omitted if we are allowed to edit data field.
        clonedData.BankAccountId = bankAccountId;
        return this._request('POST', 'mandates/directdebit/web', { body: clonedData })
            .then(mandate => this._bindMandate(mandate));
    }

    _listBankAccountMandates(userId, bankAccountId) {
        return this._request('GET', `users/${ userId }/bankaccounts/${ bankAccountId }/mandates`)
            .map(mandate => this._bindMandate(mandate));
    }

    _cancelMandate(mandateId) {
        return this._request('PUT', `mandates/${ mandateId }`)
            .then(mandate => this._bindMandate(mandate));
    }

    _createDirectDebitViaDirectPayIn(authorId, mandateId, data) {
        if(!mandateId) {
            data = mandateId;
            mandateId = undefined;
        }
        ...
        return this._createPayIn('POST', 'directdebit', 'direct', clonedData);
    }
}
